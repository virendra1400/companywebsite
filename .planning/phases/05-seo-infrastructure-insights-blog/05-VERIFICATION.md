...
| `src/app/(site)/[locale]/insights/page.tsx`, `insights/[slug]/page.tsx` | — | Route now renders dynamic (ƒ) instead of static (●) | ℹ️ Info | Side effect of the WR-03 not-found.tsx fix — Next's App Router doesn't pass route params to `not-found.tsx`, so `getTranslations()` there falls back to request-time locale detection, opting the segment out of static generation. Correct 404 status + correct copy both still hold; this is a performance tradeoff (Phase 6/PERF-01 territory), not a functional defect. |
