...
- **Global scope**: `/workspaces/companywebsite/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/workspaces/companywebsite/.claude/notes/`)
...
2. Glob `/workspaces/companywebsite/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/workspaces/companywebsite/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/workspaces/companywebsite/.claude/notes/` — works in any directory
