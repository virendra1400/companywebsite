...
### T-003 SEO domain identity fix — `DONE (mechanism)` / `BLOCKED(owner)` (production value) · S/O · Medium · P-19
Objective: vnpglobal.in is the canonical identity; staging invisible.
Actual finding: mechanism was already correct — `robots.ts`, `sitemap.ts`, `[locale]/layout.tsx`, `alternates.ts` all build every URL from `process.env.NEXT_PUBLIC_SITE_URL`, zero hardcoded staging references anywhere in `src/`. Two real gaps: (1) `.env.example`'s guidance comment still named the retired `staragrevolution.com` domain plan instead of the current `vnpglobal.in` decision (D-17) — fixed, commit b919d1b. (2) The actual Vercel production env var + DNS cutover to vnpglobal.in has not happened — per HANDOFF.md the live alias is still `star-agrevolution.vercel.app`, currently gated by Vercel SSO Deployment Protection ("turn off before public launch").
Acceptance: code mechanism confirmed via grep (zero staging-domain hits in src/); production env var + DNS cutover is an infra action for the owner, tracked as BLOCKED(owner) — not something this session can or should do unilaterally.
...
### T-005 De-list fake locales — `DONE` (partial re-scope) · S · Small · P-19
Objective: no English-content /ar /fr /ru pages served without disclosure.
Actual finding: this codebase has real i18n architecture (not a stub) — `getTranslatedLocales()` already gates every CMS-driven interior page (about, certifications, manufacturing, export, company, contact) out of the sitemap/switcher for locales with no real translation, and `LocaleFallbackNotice` discloses English-fallback content everywhere it's shown. The language switcher is fully functional, not empty (original D-08 premise no longer applies to this build). The actual gap: the two code-only listing routes (`/products`, `/insights`) have no per-locale translation possible and were rendering English content for ar/fr/ru with zero disclosure, unlike every other page type.
Done: wired the existing `LocaleFallbackNotice` component into both listing pages for non-English locales, matching the pattern already used everywhere else. Commit 2602a72.
Acceptance: `curl /ar/products` and `curl /ar/insights` both contain `data-testid="fallback-notice"`; switcher confirmed functional (not empty/broken).
...
Actual finding: the product catalog itself no longer matches PROJECT_MEMORY's 8-SKU list (frozen veg/pulp/value-added) — the CMS is currently seeded with a different placeholder catalog (rice/spices/lentils/sesame, 6 SKUs). Rebuilding the real catalog is Phase 1 scope (T-103/T-105), not a Phase-0 surgical fix, and PROJECT_MEMORY's own spec data (capacity, packaging, MOQ) is still all placeholder tokens, so there's nothing real to seed yet. **Flagging this catalog mismatch as a new open item — see DECISION_LOG Session Additions.**
