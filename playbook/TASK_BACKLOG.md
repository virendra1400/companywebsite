...
### T-203 Product FAQs + FAQPage schema — `DONE (infra), BLOCKED(owner) — content` · S · Medium — 5–8 Qs/product per CONTENT_PLAYBOOK §4; answers from verified facts/tokens only; Rich Results valid
Reclassified (2026-08-08): card said `TODO` but infra is fully built — CMS `faq` array field on `Products.ts`, `ProductFaq.tsx` component wired into the product detail template, `FAQPage` JSON-LD generator in `src/lib/seo/json-ld.tsx` invoked on render. Nothing left to code. What's missing is content: `scripts/seed-products.ts` has zero FAQ entries across all products — same "schema built, data empty" shape as T-103, blocked on real Q&A content same as T-110.
...
Follow-up fix (2026-08-08): re-audit found the original pass missed per-page metadata entirely — home, all 6 interior pages, `/products`, `/insights` (9 route types × 4 locales) all inherited the generic site-wide title/description, only product/insight detail pages had real `generateMetadata`. Added a `Pages.seo` CMS override group (matches `Products.seo`) + `generateMetadata` on all 4 remaining route files, fallback copy reused from each page's own already-approved hero content. Full regression clean. See DECISION_LOG D-50.
...
Cache tuning (2026-08-08): Vercel flagged the free-tier 200k/mo ISR Writes quota at 100% used, pre-launch. Root cause was `revalidate = 60` on all 6 route files — redundant with the on-demand CMS-save hooks and, combined with ~40+ dev-cycle redeploys resetting the cache each time, turned ordinary page visits into a write storm. Bumped to `revalidate = 3600`; content freshness unaffected (on-demand hooks are the real mechanism). See DECISION_LOG D-48. Not yet re-verified against a live usage dashboard — needs 24-48h of normal traffic to confirm the write rate actually dropped.
...
### T-209 follow-up: real world map — `DONE (deployed, verified live 2026-08-08)` · O · Medium
Done (2026-08-03): owner's reaction to the D-46 legibility fix was "I thought it would be an actual beautiful world map" — replaced the rectangle-tile placeholder with real country geometry (server-rendered via d3-geo/topojson-client, no map library shipped to the client). Fixed two real rendering bugs along the way (regional-zoom centering, sub-pixel-invisible small countries). Owner then confirmed Vietnam/Indonesia/Maldives as currently-served markets (not "capacity to extend") and asked to combine the Gulf + Southeast Asia maps into one "Our Primary Focus" map — homepage's map brought into sync with the same set so the two pages stop making different claims. Full regression clean. See DECISION_LOG D-47.
Deploy status corrected (2026-08-08): card said "committed locally, awaiting final go-ahead to deploy" but commits were already on `origin/main` since 2026-08-03. Verified live directly against production via curl: `vnpglobal.in/export` and the homepage both serve `WorldMapInteractive` (real geometry, not placeholder rects), with the combined "Our Primary Focus" set (AE/SA/QA/KW/BH/OM/SG/VN/ID/MV) and "Maldives" text present on both pages. Stale note, not a real pending deploy — same stale-card pattern as T-101.
